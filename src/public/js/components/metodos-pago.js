...
export class MetodosPagoComponent {
  ...
  view(id) {
    ...
    this.app.showDetail('Método de pago', `<div class="detail-list"><div><span>Nombre</span><strong>${metodo.nombre}</strong></div><div><span>Descuento</span><strong>${formatPercent(metodo.descuento)}</strong></div><div><span>Bonificación</span><strong>${formatPercent(metodo.bonificacion)}</strong></div></div>`);
  }
  ...
  openNew() {
    ...
    form.set('metodo-pago-bonificacion', 0);
  ...
  edit(id) {
    ...
    form.set('metodo-pago-bonificacion', metodo.bonificacion || 0);
  ...
  async save() {
    ...
    const metodo = { id, nombre, descuento: parsePercent('metodo-pago-descuento'), bonificacion: parsePercent('metodo-pago-bonificacion') };
